...
export const OverviewPage: React.FC<OverviewPageProps> = ({ metrics }) => {
  const { totalRevenueVnd, totalRevenueUsd, totalRevenueCny, topSellerProducts, userAccountsSummary, timeframe } = metrics;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between p-3.5 rounded-lg border border-slate-200 bg-slate-50 text-xs text-slate-600">
        <div className="flex items-center gap-2">
          ...
          <span><strong>View-Only Permission Mode:</strong> Showing live aggregate analytics for <strong>{timeframe.toUpperCase()}</strong> report.</span>
        </div>
        <Badge variant="outline" className="bg-white">
          <Calendar className="w-3 h-3 mr-1 text-slate-500" />
          {timeframe === 'weekly' ? 'Last 7 Days' : 'Current Month'}
        </Badge>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                topSellerProducts.map((item, index) => (
                  <div key={item.product.id} className="p-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <div className="font-semibold text-sm text-slate-900 line-clamp-1">{item.product.name}</div>
                        <div className="text-xs text-slate-500">{item.product.categories?.[0] || 'General'} • {formatVND(parseInt(item.product.pricing?.vnd || '0') || 0)}</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-bold text-slate-900">{item.totalPaidUsers} Paid Users</div>
        ...
        {/* User Account Status & Newly Created Stream */}
        <div className="space-y-6">
            ...
            <Card className="border-slate-200 bg-white">
              <CardContent className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <span className="text-2xl font-bold text-slate-900">{userAccountsSummary.activeCount}</span>
                </div>
            ...
            <Card className="border-slate-200 bg-white">
              <CardContent className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <span className="text-2xl font-bold text-slate-900">{userAccountsSummary.bannedCount}</span>
                </div>
          ...
          <Card className="border-slate-200 bg-white">
            ...
            <CardContent className="p-0">
              <Table>
                ...
                <TableBody>
                  {userAccountsSummary.newlyCreated.length === 0 ? (
                    <TableRow>
                    ...
                    userAccountsSummary.newlyCreated.map(usr => (
                    <TableRow key={usr.id}>
                      <TableCell className="font-medium">
                        <div className="text-slate-900">{usr.name}</div>
